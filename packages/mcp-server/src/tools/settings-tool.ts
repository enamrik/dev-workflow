...
export class SettingsTool {
  ...
  private async enableGitHub(github?: UpdateSettingsInput["github"]) {
    ...
    const repoUrl = await this.githubCLI.getRepoUrl();
    ...
    this.dbSource.projects.update(this.project.id, { githubSync: syncConfig });
  ...
  private async configureGitHub(github?: UpdateSettingsInput["github"]) {
    ...
    await this.dbSource.projects.update(latestProject.id, { githubSync: updatedConfig });

...
import { isValidStatusTransition, getAllowedTransitions } from "./task.js";
...
export class DrizzleTaskRepository implements TaskRepository {
  ...
  updateSyncState(taskId: string, syncState: SyncState): Task {
    const now = new Date().toISOString();
  ...
  clearSyncState(taskId: string): Task {
    const now = new Date().toISOString();

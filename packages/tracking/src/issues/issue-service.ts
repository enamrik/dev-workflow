...
import type { Issue, IssueRepository, IssueStatus } from "./issue.js";
import { isIssueClosed } from "./issue.js";
...
export class IssueService {
  ...
  async closeIssue(issueId: string, force = false, closedBy?: string): Promise<CloseIssueResult> {
    ...
    const updatedSyncState = await this.projectManagement.closeIssue(issue.syncState);
    if (updatedSyncState) {
      this.db.issues.update(issue.id, { syncState: updatedSyncState });
    }

...
export class PlanningService extends Service<PlanningService>()("planningService") {
   ...
   */
  generatePlan(request: GeneratePlanRequest): Effect<PlanWithTasks> {
    const self = this;
    return Effect.gen(function* () {
      ...
      self.dagValidationService.validateDAG(
        newTaskDefs.map((t) => ({
          id: t.id,
          title: t.title,
      ...
      const existingPlan = yield* self.db.plans.findByIssueId(issueId);
      const existingTasks = existingPlan
        ? yield* self.db.tasks.findByPlanId(existingPlan.id, false) // Exclude deleted
        : [];
      ...
      let plan: Plan;
      if (existingPlan) {
        plan = yield* self.db.plans.update(existingPlan.id, {
          summary,
          approach,
          estimatedComplexity,
          generatedBy,
        ...
        plan = yield* self.db.plans.create({
          issueId,
          summary,
          approach,
          estimatedComplexity,
  ...
  updateIssue(
    ...
    return Effect.gen(function* () {
      ...
      // Update the issue
      const updatedIssue = yield* self.db.issues.update(issueId, updates);
  ...
  private createTasksWithMatching(
    ...
    return Effect.gen(function* () {
      ...
      const matchResults = self.taskMatchingService.matchTasks(newTaskDefs, existingTasks);
      ...
      for (const task of existingTasks) {
        if (!matchedTaskIds.has(task.id) && !task.isActive && !task.isTerminal) {
          yield* self.db.tasks.softDelete(task.id, generatedBy);
        }
      }
      ...
      for (const result of matchResults) {
        if (result.action === "PRESERVE" && result.matchedTask) {
          ...
          const updatedTask = yield* self.db.tasks.update(result.matchedTask.id, {
  ...
  pauseIssue(issueNumber: number): Effect<{ count: number; tasks: Task[] }> {
    ...
    return Effect.gen(function* () {
      ...
      const allTasks = yield* self.db.tasks.findByPlanId(plan.id, false);
      ...
      if (movedTasks.length > 0) {
        self.eventBus.emit("issue:paused", {
          issueId: issue.id,
          issueNumber: issue.number,
  ...
  readyIssue(issueNumber: number): Effect<{ count: number; tasks: Task[] }> {
    ...
    return Effect.gen(function* () {
      ...
      // Emit event for real-time UI updates
      if (movedTasks.length > 0) {
        self.eventBus.emit("issue:readied", {
          issueId: issue.id,
          issueNumber: issue.number,
          tasksMovedCount: movedTasks.length,
        });
  ...
  private createNewTasks(
    ...
    return Effect.gen(function* () {
      ...
      return yield* self.db.tasks.createMany(taskData);
    });
  }
  ...
  private renumberTasks(planId: string): Effect<Task[]> {
    ...
    return Effect.gen(function* () {
      ...
      const tasks = yield* self.db.tasks.findByPlanId(planId, false);
      ...
      for (let i = 0; i < tasks.length; i++) {
        ...
        if (task.number !== newNumber) {
          const updated = yield* self.db.tasks.updateNumber(task.id, newNumber);

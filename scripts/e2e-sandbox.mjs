 ...
 *   - DWF_HOME           → dev-workflow's data root (DBs, project configs, worktrees)
...
const sandbox = mkdtempSync(join(tmpdir(), "dwf-sandbox-"));
const dwfHome = join(sandbox, "data");
...
for (const d of [dwfHome, claudeDir, proj, nonProj, binDir]) mkdirSync(d, { recursive: true });
...
try {
  ...
  const projectsDir = join(dwfHome, "projects");
  const projects = existsSync(projectsDir) ? readdirSync(projectsDir) : [];
  check("data root populated under DWF_HOME", projects.length > 0, `projects=[${projects}]`);
  check(
  ...
  const skills = existsSync(skillsDir)
    ? readdirSync(skillsDir).filter((d) => d.startsWith("dwf-"))
    : [];

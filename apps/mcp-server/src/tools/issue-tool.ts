...
export class IssueTool {
  ...
  async closeIssue(input: CloseIssueInput) {
    ...
    if (issue.sourceExternalId) {
      await this.githubCLI.closeIssue(Number(issue.sourceExternalId));
    ...
    if (parentIssueClosed) {
      message += ` Parent GitHub issue #${issue.sourceExternalId} also closed.`;
    }
    ...
    return {
      ...
      parentGitHubIssueClosed: parentIssueClosed ? issue.sourceExternalId : undefined,
    };
  }

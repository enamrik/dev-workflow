...
export class PlanTool {
  ...
  async moveIssueToReady(input: MoveIssueToReadyInput): Promise<MoveIssueToReadyResult> {
    ...
    for (const task of result.tasks) {
      await this.taskService.syncTaskStatus(task.id, "READY");
    }
  ...
  async moveIssueToBacklog(input: MoveIssueToBacklogInput): Promise<MoveIssueToBacklogResult> {
    ...
    // Use TaskService unless user explicitly skipped GitHub sync
    if (!skipGitHubSync) {
      const result = await this.taskService.activatePlannedTasks(issue.id);
    ...
    for (const task of plannedTasks) {
      await this.taskService.moveToBacklog(task.id, "system");
      activatedTasks.push({
        taskId: task.id,
   ...
   */
  async repairIssue(input: SyncIssueInput): Promise<SyncIssueResult> {
    const { issueNumber } = input;

    if (!this.taskService.isSyncEnabled()) {
      throw new Error("GitHub sync is not enabled for this project");
    }

    const result = await this.taskService.repairIssue(issueNumber);

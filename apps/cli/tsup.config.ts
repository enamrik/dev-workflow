...
export default defineConfig({
  ...
  // Stamp the release version into the bundle. CI sets DFL_VERSION (from semantic-release's
  // computed next version) before building; dev builds fall back to a dev marker. main.ts
  // reads __DFL_VERSION__ for `dev-workflow --version`.
  define: {
    __DFL_VERSION__: JSON.stringify(process.env["DFL_VERSION"] ?? "0.0.0-dev"),
  },

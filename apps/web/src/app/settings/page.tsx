...
function SettingsPageContent() {
  ...
  const syncConfig = selectedProject.syncConfig;
  const isGitHubEnabled = syncConfig?.enabled ?? false;
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
            ...
            {syncConfig?.projectId && (
              <div>
                ...
                <dd className="text-sm text-gray-900 mt-1 font-mono">{syncConfig.projectId}</dd>
              </div>
            )}
            {syncConfig?.projectUrl && (
              <div>
                <dt className="text-sm text-gray-500">Project Board</dt>
                <dd className="text-sm text-gray-900 mt-1">
                  <a
                    href={syncConfig.projectUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-blue-600 hover:text-blue-800 hover:underline"
                  >

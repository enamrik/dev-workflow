...
export class TaskAppService {
  ...
  async transitionTask(
    ...
    // Dispatch to the appropriate business method based on target status
    let updatedTask;
    switch (toStatus) {
      case "BACKLOG":
        updatedTask = await taskService.moveToBacklog(taskId, actor);
        break;
      case "READY":
        updatedTask = await taskService.moveToReady(taskId, actor);
        break;
      case "IN_PROGRESS":
        updatedTask = await taskService.start(taskId, actor);
        break;
      case "PR_REVIEW":
        updatedTask = await taskService.submitForReview(taskId, { changedBy: actor });
        break;
      case "COMPLETED":
        updatedTask = await taskService.complete(taskId, { changedBy: actor });
        break;
      case "ABANDONED": {
        // Should use abandonTask method instead
        const result = await taskService.abandonTask(taskId, "Abandoned via web UI", actor);
        updatedTask = result.task;
        break;
      }
      default:
        throw new BusinessRuleError(`Unsupported status transition to ${toStatus}`);
    }
  ...
  private createTaskService(db: DbClient): TaskService {
    ...
    return new TaskService(db, projectManagement, null);
  }

  private createIssueService(db: DbClient, taskService: TaskService): IssueService {
    const projectManagement = new ProjectManagementService(new NoOpProjectManagementClient());
    return new IssueService(db, taskService, projectManagement);
  }
}

 ...
 * These mirror the server-side types from @dev-workflow/tracking.
 */
...
export interface Issue {
  ...
  /** Sync state (optional - only present if synced to external provider) */
  syncConfig?: GitHubSyncState;
  createdAt: string;
  updatedAt: string;
}
...
export interface Project {
  ...
  gitRoot?: string;
  /** Sync configuration (optional - only present if configured) */
  syncConfig?: GitHubIssueSyncConfig | null;
}
